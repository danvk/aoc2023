...
fn parseGame(txt: []const u8) !Game {
    ...
    const parts = util.splitIntoBuf(txt, ", ", &buf);
    var intBuf: [1]u32 = undefined;
    for (parts) |part| {
        const nums = try util.extractIntsIntoBuf(u32, part, &intBuf);
        assert(nums.len == 1);
        const num = nums[0];
...
fn isLineValid(line: []const u8) !?u32 {
    ...
    var intBuf: [1]u32 = undefined;
    const ids = try util.extractIntsIntoBuf(u32, parts[0], &intBuf);
    assert(ids.len == 1);
    const id = ids[0];

    const games = util.splitIntoBuf(parts[1], "; ", &buf);
    for (games) |gameStr| {
...
test "parse game" {
    const g = try parseGame("8 green, 6 blue, 20 red");
